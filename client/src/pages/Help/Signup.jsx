...
const Form = ({
  ...
  return (
    <div className=" h-[400px] p-[20px] mt-[200px] flex items-center justify-center">
        <div className=" bg-[#ececec] w-[600px] p-[50px] shadow-lg rounded-lg flex flex-col justify-center items-center">
            ...
            <Button label={isSignInPage ? 'Sign in': 'Sign up'} type='submit' className=" bg-black w-[75%] mb-2" />
            </form>
            <div>{ isSignInPage ? "Didn't have an account?" : "Alredy have an account?"} <span className=" text-primary cursor-pointer underline" onClick={() => navigate(`/${isSignInPage ? 'Signup' : 'Signin'}`)}>{ isSignInPage ? 'Sign up' : 'Sign in'}</span></div>
        </div>
    </div>
  )
}
